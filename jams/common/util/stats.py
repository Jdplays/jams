...
from common.models import db, Attendee, AttendeeCheckInLog, EventStats, Event, Session, AttendeeSignup
...
def calculate_retention_rate(event_id, grace_threshold=0.9, min_threshold=0.5):
    ...
    retention_scores = []
    for time_attended in attendance_durations:
        retention = time_attended / event_duration
...
def calculate_workshop_dropout_rates(event_id):
    ...
    for session in sessions_in_event:
        ...
        if session.workshop.attendee_registration:
            ...
            dropout_count = sum(1 for signup in signups if signup.attendee_id in latest_checkouts)
            dropout_ratio = dropout_count / len(signups) if len(signups) > 0 else 0
            workshops_raw_data.append({
                'id': session.workshop.id,
                'name': session.workshop.name,
...
def calculate_workshop_overlap(event_id):
    ...
    for timeslot, workshops in workshops_by_timeslot.items():
        ...
        max_score = max(w['pull_score'] for w in workshops) or 1
        for workshop in workshops:
            workshop['normalised_score'] = round(workshop['pull_score'] / max_score, 2)
