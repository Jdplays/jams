...
from common.models import db, Attendee, Event, TaskSchedulerModel, Webhook, ExternalAPILog
from common.util.enums import TaskActionEnum, WebhookActionEnum, WebhookOwnerEnum, AttendeeSource
...
def get_organisations(custom_token=None):
    ...
    organisationsJson = response.json()['organizations']
    organisations = []
    for org in organisationsJson:
        org = EventbriteOrganisation(org['id'], org['name'], org['image_id'])
        organisations.append(org)
    
    return organisations

def retrieve_media(media_id, width=20, height=20):
...
def get_events(continuation_token=None):
    organisation_id = get_config_value(ConfigType.EVENTBRITE_ORGANISATION_ID)
    url = f'organizations/{organisation_id}/events/?order_by=start_desc'
...
class EventbriteOrganisation():
    ...
    def get_image(self, width=20, height=20):
        return retrieve_media(self.image_id, width, height)
