...
from web.util.helper import fetch_event_sessions
...
def archive_user(user_id):
    if current_user.id == user_id:
        abort(400, description='User cannot archive them self')
    user = User.query.filter_by(id=user_id).first_or_404()
    user.archive()
    db.session.commit()
...
def delete_event_location(event_id, event_location_id):
    ...
    if not helper.prep_delete_event_location(event_location_id):
        abort(500, description="An error occurred when trying to remove event_location")
...
def delete_event_timeslot(event_id, event_timeslot_id):
    ...
    if not helper.prep_delete_event_Timeslot(event_timeslot_id):
        abort(500, description="An error occurred when trying to remove event_timeslot")
