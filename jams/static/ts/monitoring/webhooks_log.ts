...
import { getWebhookLogs } from '@global/endpoints'
...
function initialiseAgGrid() {
    ...
    const gridElement = document.getElementById('webhooks-log-data-grid')
    gridElement.style.height = `${window.innerHeight * 0.8}px`;
    gridApi = createGrid(gridElement, gridOptions)

    populateWebhookLogsTable()
}

async function populateWebhookLogsTable() {
    const logsDataSource = {
        ...
        getRows: async function (params:any) {
            ...
            let response = await getWebhookLogs(queryString)
